...
: GuiOverlay(owner, "DAMCON_SCREEN", colorConfig.background)
{
    ...
    hull_display = new GuiKeyValueDisplay(system_health_layout, "HULL", 0.8, LanguageManager::get("Hull"), "0%");
    hull_display->setSize(GuiElement::GuiSizeMax, 40);

    for(unsigned int n=0; n<SYS_COUNT; n++)
    {
        system_health[n] = new GuiKeyValueDisplay(system_health_layout, "DAMCON_HEALTH_" + string(n), 0.8, LanguageManager::get(getSystemName(ESystem(n))), "0%");

...
: GuiOverlay(owner, "RELAY_SCREEN", colorConfig.background), mode(TargetSelection)
{
    ...
    // Reputation display.
    info_reputation = new GuiKeyValueDisplay(option_buttons, "INFO_REPUTATION", 0.7, LanguageManager::get("Reputation:"), "");
...
void RelayScreen::onDraw(sf::RenderTarget& window)
{
    ...
    if (my_spaceship)
    {
        ...
        launch_probe_button->setText(LanguageManager::get("Launch probe (") + string(my_spaceship->scan_probe_stock) + ")");
    }

...
: GuiOverlay(owner, "ENGINEERING_SCREEN", colorConfig.background), selected_system(SYS_None)
{
    ...
    for(int n=0; n<SYS_COUNT; n++)
    {
        ...
        info.button = new GuiToggleButton(info.layout, id + "_SELECT",LanguageManager::get(getSystemName(ESystem(n))), [this, n](bool value){
    ...
    power_label = new GuiLabel(box, "POWER_LABEL", LanguageManager::get("Power"), 30);
    power_label->setVertical()->setAlignment(ACenterLeft)->setPosition(20, 20, ATopLeft)->setSize(30, 360);
    coolant_label = new GuiLabel(box, "COOLANT_LABEL", LanguageManager::get("Coolant"), 30);
...
void EngineeringScreen::onDraw(sf::RenderTarget& window)
{
    if (my_spaceship)
    {
        ...
        if (selected_system != SYS_None)
        {
            ...
            power_label->setText(LanguageManager::get("Power")+ ": " + string(int(system.power_level * 100)) + "%/" + string(int(system.power_request * 100)) + "%");
            coolant_label->setText(LanguageManager::get("Coolant")+ ": " + string(int(system.coolant_level / PlayerSpaceship::max_coolant * 100)) + "%/" + string(int(system.coolant_request / PlayerSpaceship::max_coolant * 100)) + "%");

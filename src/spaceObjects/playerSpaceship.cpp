...
string alertLevelToString(EAlertLevel level)
{
    ...
    switch(level)
    {
    case AL_RedAlert: return LanguageManager::get("RED ALERT");
    case AL_YellowAlert: return LanguageManager::get("YELLOW ALERT");
    case AL_Normal: return LanguageManager::get("Normal ");
    default:
        return "???";
    }
}
...
void PlayerSpaceship::addToShipLog(string message, sf::Color color)
{
    ...
    ships_log.emplace_back(string(engine->getElapsedTime(), 1) + string(": "), LanguageManager::get(message), color);
}
        ...
        if (comms_state == CS_OpeningChannel && comms_target)
        {
            ...
            if (player_ship)
            {
                if (player_ship->comms_state == CS_BeingHailed && player_ship->comms_target == this)
                {
                    ...
                    player_ship->addToShipLog(LanguageManager::get("Hailing from ") + getCallSign() + LanguageManager::get(" stopped"), colorConfig.log_generic);
                }
            }
        }
    ...
    case CMD_OPEN_TEXT_COMM:
        if (comms_state == CS_Inactive || comms_state == CS_BeingHailed || comms_state == CS_BeingHailedByGM || comms_state == CS_ChannelClosed)
        {
            ...
            if (comms_target)
            {
                ...
                addToShipLog(LanguageManager::get("Hailing: ") + comms_target_name, colorConfig.log_generic);
            }else{
                comms_state = CS_Inactive;
            }
        }

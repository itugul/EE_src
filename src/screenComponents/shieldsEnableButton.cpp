...
: GuiElement(owner, id)
{
    button = new GuiToggleButton(this, id + "_BUTTON", LanguageManager::get(" Shields: ON"), [](bool value) {
        if (my_spaceship)
    ...
    if (my_spaceship)
    {
        ...
        else
        {
            ...
            if (gameGlobalInfo->use_beam_shield_frequencies)
                button->setText(frequencyToString(my_spaceship->shield_frequency) + (LanguageManager::get(my_spaceship->shields_active ? " Shields: ON" : " Shields: OFF")));
            else
                button->setText(LanguageManager::get(my_spaceship->shields_active ? " Shields: ON" : " Shields: OFF"));
        }
    }
}

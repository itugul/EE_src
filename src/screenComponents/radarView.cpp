...
void GuiRadarView::drawObjects(sf::RenderTarget& window_normal, sf::RenderTarget& window_alpha)
{
    ...
    if (my_spaceship)
    {
        ...
        if (show_callsigns && my_spaceship->getCallSign() != "")
            drawText(window_normal, sf::FloatRect(object_position_on_screen.x, object_position_on_screen.y - 23, 0, 0), my_spaceship->getCallSign(), ACenter, 15, bold_font);
    }
}

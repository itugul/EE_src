...
: GuiAutoLayout(owner, id, LayoutVerticalBottomToTop), load_type(MW_None), manual_aim(false), missile_target_angle(0)
{
    ...
    for (int n = MW_Count-1; n >= 0; n--)
    {
        ...
        load_type_rows[n].button = new GuiToggleButton(load_type_rows[n].layout, id + "_MW_" + string(n), LanguageManager::get(getMissileWeaponName(EMissileWeapons(n))), [this, n](bool value) {
            if (value)
...
void GuiMissileTubeControls::onDraw(sf::RenderTarget& window){
    ...
    {
        load_type_rows[n].button->setText(LanguageManager::get(getMissileWeaponName(EMissileWeapons(n))) + " [" + string(my_spaceship->weapon_storage[n]) + "/" + string(my_spaceship->weapon_storage_max[n]) + "]");
    ...
    for (int n = 0; n < my_spaceship->weapon_tube_count; n++)
    {
        ...
        if(tube.isEmpty())
        {
            ...
            rows[n].fire_button->setText(tube.getTubeName() + LanguageManager::get(": Empty"));
        ...
        }else if(tube.isLoaded())
        {
            ...
            rows[n].fire_button->setText(tube.getTubeName() + ": " + LanguageManager::get(getMissileWeaponName(tube.getLoadType())));
            rows[n].loading_bar->hide();
        ...
        }else if(tube.isUnloading())
        {
            ...
            rows[n].load_button->setText("Unload");
            rows[n].fire_button->hide();
            rows[n].fire_button->setText(LanguageManager::get(getMissileWeaponName(tube.getLoadType())));
            rows[n].loading_bar->show();

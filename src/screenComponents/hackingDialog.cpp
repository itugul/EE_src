...
void GuiHackingDialog::onDraw(sf::RenderTarget& window)
{
    ...
    if (target_system != "")
    {
        ...
        for(std::pair<string, float>& target : targets)
        {
            ...
            {
                hacking_status_label->setText(LanguageManager::get("Hacked ") + LanguageManager::get(target_system) + ": " + string(int(target.second * 100.0f + 0.5f)) + "%");
                break;
            }
        }
    }
...
void GuiHackingDialog::onFieldClick(int x, int y)
{
    ...
    }else if (correct_count == (hacking_field_size * hacking_field_size - bomb_count))
    {
        ...
        status_label->setText(LanguageManager::get("Hacking in progress: ") + string(correct_count * 100 / (hacking_field_size * hacking_field_size - bomb_count)) + "%");

...
OptionsMenu::OptionsMenu()
{
    ...
    // Language settings.
    top += 60;
    (new GuiLabel(this, "LANGUAGE_LABEL", "Language", 30))->addBackground()->setPosition(50, top, ATopLeft)->setSize(300, 50);

    // Determine when music is enabled.
    int language_index = PreferencesManager::get("language", "0").toInt();
    top += 50;
    (new GuiSelector(this, "LANGUAGE", [](int index, string value)
    {
        // 0: English(default)
        // 1: Russian
        PreferencesManager::set("language", string(index));
    }))->setOptions({"English", "Russian"})->setSelectionIndex(language_index)->setPosition(50, top, ATopLeft)->setSize(300, 50);
    PreferencesManager::set("language", string(language_index));
